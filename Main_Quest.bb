...
Function main_menu()
  ...
  Local backgrnd = LoadImage("bd01.bmp")
  ...
  EndIf
    ...
    If KeyHit(208) Then mi# = mi# + 1 ; down arrow

    If KeyHit(200) Then mi# = mi# - 1 ; up arrow

    If mi# > OptionsCnt Then mi# = 1

    If mi# < 1 Then mi# = OptionsCnt
...
Function update_quests()
  ...
  ElseIf (q\Active# = 1) And (q\Done# = 0) ;If it's active, but haven't been done
    ...
    ElseIf (q\Condition# = 3) ;If we must bring smthng to smbd
      ...
      For b.TBot = Each TBot ;Searching for this bot
        If (b\Name$ = q\SValue2$) And (EntityDistance(b\Entity#, user\Entity#) <= user\PickR#*5)
        q\Active# = 0
        q\Done# = 1
        message(q\SValue0$+": quest done")
        Exit
...
Function message(msg$)
  Local backgrnd = LoadImage("message.bmp")
  MaskImage backgrnd, 0, 0, 0
  Local font = LoadFont("Arial", 16, 1)
  Local font0 = LoadFont("Arial", fs, 0)
  SetFont font
...
Function question(msg$, a1$, a2$)
  ...
  If KeyHit(203) Then Return True
